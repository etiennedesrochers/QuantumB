...
import src.core.rules_manager as rl
...
import src.core.valve_manager as vm
...
class MainWindow(QMainWindow):
    ...
    def _build_template_tab(self) -> QWidget:
        ...
        self._btn_export_tmpl = QPushButton()
        self._btn_export_tmpl.clicked.connect(self._export_templates)
        ...
        t0_btn_lay.addWidget(self._btn_export_tmpl)
        ...
        self._btn_export_ctrl_tmpl = QPushButton()
        self._btn_export_ctrl_tmpl.clicked.connect(self._export_ctrl_templates)
        ...
        t1_btn_lay.addWidget(self._btn_export_ctrl_tmpl)
        ...
        self._btn_export_io_tmpl = QPushButton()
        self._btn_export_io_tmpl.clicked.connect(self._export_io_templates)
        ...
        t2_btn_lay.addWidget(self._btn_export_io_tmpl)
        ...
        self._btn_export_ladder_tmpl = QPushButton()
        self._btn_export_ladder_tmpl.clicked.connect(self._export_ladder_templates)
        ...
        self._btn_export_valves_tmpl = QPushButton()
        self._btn_export_valves_tmpl.clicked.connect(self._export_valves_templates)
        ...
        t4_btn_lay.addWidget(self._btn_export_valves_tmpl)
        ...
        self._btn_export_ladder_component_tmpl = QPushButton()
        self._btn_export_ladder_component_tmpl.clicked.connect(self._export_ladder_component_templates)
    ...
    def _load_io_types(self):
        path = Path(__file__).resolve().parent.parent.parent / "config" / "io_types_library.json"
        try:
            if path.exists():
    ...
    def _add_valve_io(self):
        ...
        if dlg.exec() == QDialog.Accepted and dlg.result_data:
            from src.core.models import ValveIO
            ios = vm.get_ios_for_type(vtype)
    ...
    def _sort_io_items(
        ...
        try:
            file_path = Path(__file__).resolve().parent.parent.parent / "order_file" / "Order_IO.xlsx"
    ...
    def _export_templates(self):
        """Export all regular templates to Excel file."""
        self._export_templates_to_excel(
            self._template_mgr,
            "Export Templates",
            "Templates.xlsx"
        )

    def _export_ctrl_templates(self):
        """Export all controller templates to Excel file."""
        self._export_templates_to_excel(
            self._ctrl_template_mgr,
            "Export Controller Templates",
            "Controller_Templates.xlsx"
        )

    def _export_io_templates(self):
        """Export all IO templates to Excel file."""
        self._export_templates_to_excel(
            self._io_template_mgr,
            "Export IO Templates",
            "IO_Templates.xlsx"
        )

    def _export_ladder_templates(self):
        """Export all ladder templates to Excel file."""
        self._export_templates_to_excel(
            self._ladder_template_mgr,
            "Export Ladder Templates",
            "Ladder_Templates.xlsx"
        )

    def _export_valves_templates(self):
        """Export all valves templates to Excel file."""
        self._export_templates_to_excel(
            self._valves_template_mgr,
            "Export Valves Templates",
            "Valves_Templates.xlsx"
        )

    def _export_ladder_component_templates(self):
        """Export all ladder component templates to Excel file."""
        self._export_templates_to_excel(
            self._ladder_component_template_mgr,
            "Export Ladder Component Templates",
            "Ladder_Component_Templates.xlsx"
        )

    def _export_templates_to_excel(self, template_mgr, dialog_title: str, default_filename: str):
        """Generic method to export templates from any template manager to Excel."""
        # Get all template names
        templates = template_mgr.list_templates()
        
        if not templates:
            QMessageBox.warning(self, tr("msg_validation"), "No templates to export.")
            return
        
        # Ask user where to save the file
        file_path, _ = QFileDialog.getSaveFileName(
            self, dialog_title, default_filename,
            "Excel Files (*.xlsx);;All Files (*.*)"
        )
        ...
        try:
            # Create DataFrame with template names
            df = pd.DataFrame({"Template Name": sorted(templates)})
            
            # Write to Excel
            df.to_excel(file_path, sheet_name="Templates", index=False)
            
            QMessageBox.information(
                self, tr("msg_success_title"),
                f"Templates exported successfully to:\n{file_path}\n\n{len(templates)} templates exported."
            )
        except Exception as e:
            QMessageBox.critical(self, tr("msg_error_title"), f"Error exporting templates:\n{str(e)}")
    ...
    def _retranslate_ui(self) -> None:
        ...
        self._btn_export_tmpl.setText(tr("btn_export"))
        ...
        self._btn_export_ctrl_tmpl.setText(tr("btn_export"))
        ...
        self._btn_export_io_tmpl.setText(tr("btn_export"))
        ...
        self._btn_export_ladder_tmpl.setText(tr("btn_export"))
        ...
        self._btn_export_valves_tmpl.setText(tr("btn_export"))
        ...
        self._btn_export_ladder_component_tmpl.setText(tr("btn_export"))
